...
| 作業画面（部品表示） | 全画面に 1 品目。外周に残り割合のリング（進んだぶんは種類の色＋うっすら光、未達は灰色、下は経過時間のために空ける）、その内側は種類の色を暗く落とした地。中身は上から 種類バッジ（種類名＋立方体アイコン）/ 機種名（枠に収まらなければ横へ流す）/ PL・CT / PCS。上に現在時刻、下に経過時間（橙）|
| 作業画面（積み方） | 種類バッジの立方体アイコンを押すと出る。端数パレットを積み終わった形で、中央から回りながら大きくなって出てくる。そのあとは勢いよく回り始めてだんだん落ち着く速さで回り続ける（スマホ版の全画面表示と同じ 1.4 秒・260→24 度/秒・時定数 1.8 秒）。どこかをタップすると閉じる |
| 作業画面（一覧） | 角を丸めたバーで、左 3/5 に品名（収まらなければ横へ流す）、右 2/5 に PL / CT / PCS（数字は大きく、単位はごく小さく）。タップするとその品目に切り替わり、部品表示へ戻る |
| 作業画面（人物出現） | コールの選択の「人物出現」を押すと出る、誰を出すかの選び画面（山本将 / 小谷 / 長村）。押すとスマホの一覧のところにその人が出る。名前と写真はスマホ側（`src/lib/people.ts`）が持っていて、ウォッチは ID を送るだけ |
| 作業画面（品目の詳細） | 一覧のバーを長押しすると出る。種類の色をうっすら敷いた地に、角を丸めた囲みを積み重ねる。上から 何番目か / 種類・状態 / 色・鍋のサイズ / 機種名（種類の色でうっすら光る）/ 気高コード（KTE・青）・新建高コード（KEN・赤）/ 進み（％とバー、済み・残り・元の数）/ 残り（PL・CT・PCS）/ 1 箱（入数・1PL・1 段・外寸・積み方）/ 重さ・かさ（1 箱・1PL・残り合計）/ 代表機種と英語名 / 注意書き（類似品あり。橙）/ 端数パレットの積み方。リューズでも指でも動かせる。2 回タップで一覧へ戻る |

**操作は縦だけ**にしてある。横スワイプはウォッチの「戻る」がそのまま働く（アプリが閉じる）。

- 部品表示で縦に払う … 品目を切り替える（上へ払うと次、下へ払うと前。端まで行くと反対の端へ回る）。
  荷降ろしが済んだ品目は飛ばす（終わった部品が出てこない）
- 部品表示のいちばん下から上へ払う … 一覧を開く
- 部品表示でリューズを時計回りに多めに回す … 一覧を開く（3 目盛りほど。逆に回すか手を止めると数え直す）
- 一覧のいちばん上からさらに下へ、またはいちばん下からさらに上へ払う … 部品表示へ戻る
- 部品表示の上のほう（時刻・気温・種類バッジ）を長押し … コールの選択を出す
- 部品表示のそれより下（機種名・PL・CT・PCS）を長押し … その品目の詳細を見る
- 一覧のバーを長押し … その品目の詳細を見る
- 種類バッジの立方体アイコンを押す … 端数パレットの積み方を見る

一覧と詳細はリューズでも動かせる。一覧の右端の弧は、どのあたりを見ているかの目印
（画面の縁に沿った 42 度ぶん）。

積み方の並べ方は、スマホ側（`src/components/PalletDiagram.tsx`）と同じ決まりを `shared` の
`PalletLayout.kt` に写してある（鍋・ポリカバーの 3 列積み＝1 段 6 個、入数 9 個以上の大きい内鍋は
100・60 サイズが 奥に横 2 個（左右）＋手前に縦 3 個（左右）の 1 段 5 個・5 段、
180 サイズが 2 列 × 2 行の 1 段 4 個・4 段、
ジャーポットの 2×2、PDU の風車状 10 玉、JPI の 1 段 7 個、そのほかは寸法どおり。
端数は四隅を残して中央から抜く）。
描くのはウォッチ側の `wear/.../ui/PalletDiagram.kt`。

文字や数字の大きさは画面の幅を基準に決めているので、時計の大きさが変わっても見え方が揃う（`BoxWithConstraints`）。
数字はスマホ版と同じように数えて見せ（品目が変わったら 0.4 秒待って 1 秒で数え上げ、値が変わったら 0.5 秒でなじませる）、
リングもなめらかに減る（`Components.kt` の `countUp` / `smoothFraction`）。
現在時刻は黒い丸枠に入れて各画面が自前で出す（`Components.kt` の `TimePill`）。
...
| 部品表示を縦スワイプ | `selectItem` | 前後の品目に切り替える（上へ払うと次、下へ払うと前。端まで行くと反対の端へ回る）。済んだ品目は飛ばす |
| コールの選択で「お願いします！」「長谷川さん！」「応援コール」「いまの品目を読み上げ」 | `call`（`request` / `name` / `cheer` / `item`）| スマホがそのコールを読み上げる |
| コールの選択で「天気コール」 | `call`（`weather`）| スマホのメニューの天気コールと同じ（読み上げ＋天気のポップアップ）|
| コールの選択で「水の音」 | `call`（`water`）| 流れる水の音を止める／流す |
| コールの選択で「せせらぎモード」 | `call`（`river`）| せせらぎモード（川の映像）を開く |
| コールの選択で「人物出現」→ 名前 | `call`（`person:<ID>`）| スマホの一覧のところに、その人が 10 秒だけ出る（ゆっくり現れて、ゆっくり消える）|
