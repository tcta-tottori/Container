...
- **操作は右下のクイックメニューに集約**: 角丸四角の中に上向きの山（シェブロン）を描いたアイコン（`QuickMenuIcon`）を押すと下から展開し、
  コンテナ選択・応援コール・天気コール・水の音・SwitchBot 接続をまとめて操作できる（`src/components/QuickActions.tsx`）。
...
- **メニューは画面左側全域**: 上端から下端まで（幅 `min(320px, 82vw)`）を占め、オーバーレイとパネルがフェードインしたあと、
  ...
  下部にバージョンと最終ビルド時刻（`Ver 4.0  11:15`）を表示する（更新内容の一覧表示は行わない）。
...
│   ├── QuickActions.tsx     # 右下の展開メニュー（コンテナ選択・各コール・水の音・SwitchBot）
