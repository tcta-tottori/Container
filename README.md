...
- **タップ**: 画面上側は波紋が広がるだけ。**画面の下 1/3 をタップすると元の画面に戻る**（Esc でも戻れる）。
...
- SwitchBot 受信中は実測値を表示する。出所を示す「S」マークは付けず、代わりに数値の左側へ
  気象庁の予報値との差分（上段=気温差 `+1.4`、下段=湿度差 `-6`）を小さく表示する。
...
- **SwitchBot 温湿度計（Bluetooth）**: 「接続」ボタンで SwitchBot 温湿度計の BLE アドバタイズをスキャンし、実測の気温・湿度・WBGT を表示。
  受信中は「S」マークなどの出所バッジは出さず、気象庁の予報値との差分（SwitchBot − 気象庁）を数値の左側に小さく重ねて表示する（上段=気温差、下段=湿度差）。
...
- **メニューは画面左側全域**: 上端から下端まで（幅 `min(320px, 82vw)`）を占め、オーバーレイとパネルがフェードインしたあと、
  ...
  下部にバージョンと最終ビルド時刻（`Ver 4.6  11:15`）を表示する（更新内容の一覧表示は行わない）。
