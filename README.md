...
- **継ぎ目のないループ**: 元動画（10秒）から、先頭と末尾が最も近くなる区間を選び、
  末尾 1.5 秒を先頭にクロスフェードして約 6.5 秒のループに作り直している（`public/videos/river-loop.mp4`）。
  ループのつなぎ目の絵の差は、通常のコマ送り1コマ分と同程度まで下がっている。
- **形式**: H.264 の MP4 と VP9 の WebM を用意し、`<source>` で MP4 を優先。音声は入れていない
  （BGM は水の音の機能を使う）。読み込み前は 1 コマ目のポスター画像（`river-poster.jpg`）を出す。
...
- **メニューは画面左側全域**: 上端から下端まで（幅 `min(320px, 82vw)`）を占め、オーバーレイとパネルがフェードインしたあと、
  ...
  下部にバージョンと最終ビルド時刻（`Ver 4.5  11:15`）を表示する（更新内容の一覧表示は行わない）。
