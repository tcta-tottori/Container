...
動かすのは**川・枝葉・靄だけ**で、地面・石・木の根元は写真のまま静止させる。
そのために写真から2つのマスクを作り、動かす部分だけを切り抜いている。

- `public/images/river-water-mask.png` … 水面（明るく彩度の低い部分・画面下側のみ）
- `public/images/river-leaf-mask.png` … 枝葉（緑が強く暗すぎない部分・画面上側のみ）
- どちらも **アルファチャンネル**に形を入れた PNG。CSS の `mask-image` は既定で画像の
  アルファを見るため、グレースケール画像のままではマスクが効かない（全面に薄く掛かってしまう）。
- マスクは**動かない外枠**（`.river-clip`）に掛け、その中の写真だけを動かす。
  マスクごと動かすと切り抜き位置がずれて、石や地面にまで動きがはみ出す。

- **川が流れる**: 水面マスクの内側で写真を下流方向へずらし、2枚を半周ずらして交互にフェードする
  （`globals.css` の `river-flow`）。写真の水は長時間露光で滑らかなので、これだけで流れて見える。
  さらに水面マスクの内側だけに、流れの筋（canvas）を薄く重ねている。
- **枝葉が揺れる**: 葉マスクの内側の写真を下端を軸に ±0.18 度ゆっくり傾ける（`river-sway`）。
  重ね合わせのにじみが出ない範囲の小さな振れ幅にしている。
- **靄**: 上流部にゆっくり漂う霧（`river-mist-drift`）。
...
- **メニューは画面左側全域**: 上端から下端まで（幅 `min(320px, 82vw)`）を占め、オーバーレイとパネルがフェードインしたあと、
  ...
  下部にバージョンと最終ビルド時刻（`Ver 4.3  11:15`）を表示する（更新内容の一覧表示は行わない）。
...
│   ├── RiverMode.tsx        # せせらぎモード（川の映像・川と枝葉と靄だけが動く）
