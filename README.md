...
### ヘッダー・メニュー・読込画面（UI）
- **ヘッダーは常に固定**: `html, body` 自体をスクロールさせない（`overflow: hidden`）構成にし、
  各ページは内側のコンテナだけがスクロールする。ヘッダーは `position: sticky` も併用して、
  どの画面でもスクロールで流れて消えないようにしている。
...
- **メニューは画面左側全域**: 上端から下端まで（幅 `min(320px, 82vw)`）を占め、オーバーレイとパネルがフェードインしたあと、
  ...
  下部にバージョンと最終ビルド時刻（`Ver 4.4  11:15`）を表示する（更新内容の一覧表示は行わない）。
