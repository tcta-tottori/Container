...
数値を大きく見せるため、出典（気象庁）と観測時刻はバーには出さず、タップで開くポップアップ側に表示する。
SwitchBot は「S」マーク付きの「接続」ボタンだけを置き、状態や対処法は長押し（またはエラー表示のタップ）で開く。
...
### ヘッダー・メニュー・読込画面（UI）
- **ヘッダーは 1.5 倍サイズ**: 高さ 72px（`calc(72px + var(--safe-top))`）、ボタン 48px 角、経過時間は 19px。
  現在時刻は端末のステータスバーに出るため、ヘッダーには表示しない（横画面の日付と経過時間のみ）。
...
- **経過時間はデジタル調フォント**: `Orbitron`（CSS 変数 `--font-clock`）+ ネオングローで表示する。
  日本語を含む日付表示は `Orbitron` に字形がないため従来どおり `--font-mono`（Inter）のままにしている。
- **メニューは画面左側全域**: 上端から下端まで（幅 `min(320px, 82vw)`）を占め、
  オーバーレイのフェードイン + パネルの微小スライドで展開する（`globals.css` の `menu-overlay-in` / `menu-panel-in`）。
  下部にバージョンと最終ビルド時刻（`Ver 3.8  11:15`）を表示する（更新内容の一覧表示は行わない）。
